...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/israel_search_tenders \
  -H 'Content-Type: application/json' \
  -d '{"query":"בריאות","limit":20}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/israel_search_tenders`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "israel-tenders": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-israel-tenders"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-israel-tenders
```
